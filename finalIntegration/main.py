...
servoEN = PWM(Pin(17), freq=50, duty_u16=2400)
servoPH = Pin(16, Pin.OUT)
trig = Pin(20, Pin.OUT)
echo = Pin(21, Pin.IN)
servoPH.value(0)

SERVO_MIN_DUTY = 2400      # "rest" position
SERVO_MAX_DUTY = 4000      # "activated" position
SERVO_STEP     = 50        # how much to change per loop
...
try:
    while True:
        ...
        distance_cm = (pulse_duration / 2) / 29.1  # Convert to cm

        print("Distance: {:.2f} cm".format(distance_cm))

        if distance_cm < 5:
            target_servo_duty = SERVO_MAX_DUTY
        else:
            target_servo_duty = SERVO_MIN_DUTY

        # ----- Smooth servo update (incremental move) -----
        if current_servo_duty < target_servo_duty:
            current_servo_duty = min(current_servo_duty + SERVO_STEP, target_servo_duty)
        elif current_servo_duty > target_servo_duty:
            current_servo_duty = max(current_servo_duty - SERVO_STEP, target_servo_duty)

        servoEN.duty_u16(current_servo_duty)
